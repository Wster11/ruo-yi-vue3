...
export default [
  // 1. 文件列表（支持分页和多维度筛选）
  {
    url: '/dev-api/v1/fileManager/list',
    method: 'get',
    response: ({ query }) => {
      ...
      const isAssociated = query.isAssociated
  ...
  // 2. 获取语言列表
  {
    url: '/dev-api/v1/fileManager/geLanguageList',
    method: 'get',
    response: () => {
      return {
        code: 200,
  ...
  // 3. 获取文件类型列表
  {
    url: '/dev-api/v1/fileManager/getDocumentTypeList',
    method: 'get',
    response: () => {
      return {
        code: 200,
  ...
  // 4. 获取分类列表
  {
    url: '/dev-api/v1/fileManager/getCategoryList',
    method: 'get',
    response: () => {
      return {
        code: 200,
  ...
  // 5. 获取设备列表
  {
    url: '/dev-api/v1/fileManager/getDeviceList',
    method: 'get',
    response: () => {
      return {
        code: 200,
  ...
  // 6. 文件新增
  {
    url: '/dev-api/v1/fileManager/add',
    method: 'post',
    response: ({ body }) => {
      console.log('新增文件请求参数:', body)
  ...
  {
    url: '/dev-api/v1/fileManager/updateFileVersio',
    method: 'get',
    response: ({ query }) => {
  ...
  // 8. 关联更多设备 - 查询设备列表
  {
    url: '/dev-api/v1/fileManager/getDeviceAllList',
    method: 'get',
    response: ({ query }) => {
      const fileId = query.fileId
  ...
  // 9. 关联更多设备 - 保存关联
  {
    url: '/dev-api/v1/fileManager/bindSelectDevice',
    method: 'post',
    response: ({ query, body }) => {
      const fileId = query.fileId
      const deviceIds = query.deviceId // 可能是逗号分隔的ID列表
  ...
  // 10. 文件详情 - 获取绑定设备列表
  {
    url: '/dev-api/v1/fileManager/getBindDeviceList',
    method: 'get',
    response: ({ query }) => {
      const fileId = query.fileId
